...
(plot1 <-  ggplot(df.tidy, aes(x=IEP, fill = soil)) + geom_density(alpha=0.7) + theme_linedraw() + 
  ...
  scale_fill_manual(values = c('burlywood1', 'darkslateblue', 'cornflowerblue')) + theme( panel.grid.major = element_line(color = "gray80"),
    panel.grid.minor = element_line(color = "gray80"), legend.position = 'none'))
...
(pic1 <- ggplot(df.tidy, aes(pH, IEP, fill = pH)) + geom_boxplot(show.legend = FALSE) + theme_linedraw() + 
  ...
  scale_fill_manual(values = c('goldenrod1','darkolivegreen3','chartreuse4', 'darkcyan')) + facet_wrap(~soil, nrow = 1, labeller = labeller(soil = c("ML" = "Grassland","AN" = "Forest", "BB" = "Wetland"))) + theme( panel.grid.major = element_line(color = "gray80"),
    panel.grid.minor = element_line(color = "gray80")))


(sfig <- plot_grid(plot1, pic1, ncol = 2, labels = c('A','B'), rel_widths = c(2, 1.5)))
ggsave('IEPDistribution_bysoil_byPH_SuppFig.png', sfig, width = 10, height = 4)
```
