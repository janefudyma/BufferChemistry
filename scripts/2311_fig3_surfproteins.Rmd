...
squn <- read_csv('../data/2309_surfaceproteins_vcontactdatabase_AAsequences.csv')
...
(plot1 <-  ggplot(df.tidy, aes(x=IEP, fill = pH)) + geom_density(alpha=0.7) + theme_linedraw() + 
  ...
  scale_fill_manual(values = c('goldenrod1','darkolivegreen3','chartreuse4', 'darkcyan'), labels = c("4.5", "5.5", "6.5", "7.5") ) + 
    ...
    panel.grid.minor = element_line(color = "gray93"), legend.position = 'bottom', legend.title = element_text(face = 'bold', size = 8), legend.text = element_text(size = 8), axis.title.x = element_text(face ="bold"),
...
(finalplot <- plot_grid(lerft, bc.plots, plot11, ncol = 3, labels = c('A','B','C'), rel_widths = c(1.1,1.6,1.5)) + theme(plot.background = element_rect(fill = "white", color = 'white')))

ggsave('../Figures/Figure3_Protein_BetaDiversity_Iso.pdf', finalplot, width = 14, height = 5, dpi = 400, device = 'pdf')
```
